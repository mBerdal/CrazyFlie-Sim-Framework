...
class CrazyFlie():
  ...
  def do_step(self, environment: Environment, time_step: float) -> None:
    avg_obstacle_vect = np.zeros((Environment.__SPATIAL_DIMS__,))
    for s in self.sensors:
      if isinstance(s, RangeSensor):
        dist = s.get_reading(environment, self.state, 0)
        if not dist is None:
          avg_obstacle_vect += dist
          
    self.state -= time_step*avg_obstacle_vect/(np.linalg.norm(avg_obstacle_vect))**2
