...
class CrazyFlie(Drone):
    ...
    def __init__(self, id, state: np.ndarray, **kwargs) -> None:
        ...
        sensors = [
            RangeSensor(
                np.array([((-1) ** i) * 0.01 if i == 0 or i == 2 else 0, ((-1) ** i) * 0.01 if i == 1 or i == 3 else 0,
                          0]).reshape(3, 1),
        ...
        self.sensor_idx = []
        start_ind = 0
        for ind, s in enumerate(self.sensors):
            if type(s) not in [LidarSensor,RangeSensor]:
                continue
            ...
            self.sensor_idx.append({"start": start_ind, "end": start_ind + r.shape[1], "sensor": ind})
            start_ind = start_ind + r.shape[1]
        self.rays = np.concatenate(rays, axis=1)
        self.origins = np.concatenate(origins, axis=1)
    ...
    def get_sensor_rays(self):
        ...
        origins = rot_body_to_ned @ self.origins + self.state[0:3]
        return rays, origins, self.max_ranges, self.min_ranges
    ...
    def update_state_dot(self, step_length):
        self.state_dot = self.state_dot + step_length * clip(self.command - self.state_dot, self.acc_limits_lower,
                                                             self.acc_limits_upper)
...
class CrazyFlieLidar(Drone):
    ...
    def __init__(self, id, state: np.ndarray, **kwargs) -> None:
        ...
        sensors = [LidarSensor(np.array([0, 0, 0.01]).reshape(3, 1), np.array([0, 0, 0]).reshape(3, 1),
                               max_range=max_range_sensor, num_rays=num_rays_sensor),
                   OdometrySensor()
                   ]
    ...
    def update_state_dot(self, step_length):
        self.state_dot = clip(self.state_dot + step_length * clip(self.command, self.acc_limits_lower, self.acc_limits_upper),self.state_dot_lower,self.state_dot_upper)
