...
class DroneSwarm(CommunicationNode):
    ...
    def read_range_sensors(self, environment):
        rays, origins, idx_drones = self.get_range_rays()
        rays = rays.transpose()
        origins = origins.transpose()
        t_min = np.ones(rays.shape[0]) * np.inf

        for obs in environment.get_obstacles():
            t = multi_ray_intersect_triangle(origins, rays, obs.points, np.max(self.max_ranges))
        ...
        for d in self.drones:
            ...
            for idx in d.get_sensor_idx():
                d.sensors[idx["sensor"]].add_measurement(local_readings[idx["start"]:idx["end"]])

    def read_odometry_sensors(self,time_step):
        for d in self.drones:
            for s in d.sensors:
                if type(s) == OdometrySensor:
                    s.add_measurement(d.state_dot, time_step)
    ...
    def sim_step(self, time_step, environment):
        ...
        self.read_odometry_sensors(time_step)
    ...
    def get_range_rays(self):
        ...
        origins = np.concatenate(origins, axis=1)
        return rays, origins, idx_drones
