...
"""
    ...
    def load_from_file(self, filename: str = ""):
        try:
            ...
            assert "trajectory" in dict(next(iter(
                drones_dict.values()))).keys(), f"Trajectories not specified in file {filename}. Failed loading from file."
            ...
            self.log["environment"] = Environment(
                [Obstacle(obj["shape"], np.array(obj["points"])) for obj in env_dict["obstacles"]])

            drones = []
            for id, drone in drones_dict.items():
                d_class = getattr(importlib.import_module(drone["info"]["module"]), drone["info"]["cls"])
                d_args = dict(filter(lambda elem: elem[0] != "module" and elem[0] != "cls" and elem[0] != "id",
                                     drone["info"].items()))
                ...
                for s in drone["info"]["sensors"]:
                    ...
                    s_args = {
                        ...
                        for k, v in
                        dict(filter(lambda elem: elem[0] != "module" and elem[0] != "cls", s.items())).items()
                    }
                    sensors.append(s_class(**s_args))

                drones.append(
                    d_class(id=id, state=np.array(next(iter(drone["trajectory"].values()))["state"]), **d_args))
            self.__log_info(drones)
    ...
    def get_sensor_plotting_kwargs(self, drone_id, sensor_idx, time):
        return {
            ...
            **dict(filter(lambda elem: elem[0] != "cls" and elem[0] != "module",
                          self.__get_drone_sensor_specs(drone_id, sensor_idx).__dict__.items())),
